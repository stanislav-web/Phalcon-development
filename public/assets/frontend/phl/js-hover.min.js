...
$rootScope.$on('$translatePartialLoaderStructureChanged',function(){$translate.refresh();});}]);})(angular);;"use strict";(function(angular){phl.constant('ROUTES',(function(){return{HOME:'/',CONTACTS:'/contacts',HELP:'/help',AGREEMENT:'/agreement',ABOUT:'/about',NOT_FOUND:'/error/notfound',SIGN:'/sign'}})());phl.constant('TEMPLATE',(function(){return{ARTICLE:'assets/frontend/phl/app/templates/index.html',ERROR:'assets/frontend/phl/app/templates/error.html',SIGN:'assets/frontend/phl/app/templates/sign.html'}})());phl.config(['$routeProvider','$locationProvider','ROUTES','TEMPLATE',function($routeProvider,$locationProvider,ROUTES,TEMPLATE){$locationProvider.hashPrefix('!');$locationProvider.html5Mode(true);$routeProvider.when(ROUTES.HOME,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.CONTACTS,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.HELP,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.AGREEMENT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.ABOUT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.NOT_FOUND,{templateUrl:TEMPLATE.ERROR,controller:'IndexController'}).otherwise({redirectTo:ROUTES.HOME});}]);phl.config(['$translateProvider','$translatePartialLoaderProvider',function($translateProvider,$translatePartialLoader){$translateProvider.fallbackLanguage(['en','ru','de']).determinePreferredLanguage().registerAvailableLanguageKeys(['en','de','ru'],{'en_US':'en','en_GB':'en','de_DE':'de','ru_RU':'ru'});$translateProvider.useLoader('$translatePartialLoader',{urlTemplate:'assets/frontend/phl/app/languages/{lang}/{part}.json'});}]);})(angular);;
angular.module('ui.splash',['ui.bootstrap']).service('$splash',['$modal','$rootScope',function($modal,$rootScope){return{open:function(attrs,opts){var scope=$rootScope.$new();angular.extend(scope,attrs);opts=angular.extend(opts||{},{backdrop:false,scope:scope,templateUrl:'splash/content.html',windowTemplateUrl:'splash/index.html'});return $modal.open(opts);}};}]).run(['$templateCache',function($templateCache){$templateCache.put('splash/index.html','<section class="splash" ng-class="{\'splash-open\': animate}" ng-style="{\'z-index\': 1000, display: \'block\'}" ng-click="close($event)">'+'  <div class="splash-inner" ng-transclude></div>'+'</section>');$templateCache.put('splash/content.html','<div class="splash-content text-center">'+'  <h1 ng-bind="title"></h1>'+'  <p class="lead" ng-bind="message"></p>'+'  <button class="btn btn-lg btn-outline" ng-bind="btnText || \'Ok, cool\'" ng-click="$close()"></button>'+'</div>');}]);;"use strict";phl.controller('TopMenuController',function($scope,$location,$translatePartialLoader,$splash,$http,$sce,$rootScope){$translatePartialLoader.addPart('menu');$scope.isActive=function(route){return route===$location.url();}
$scope.openSplash=function(url){$http.get(url).success(function(data){$scope.message=function(){return $sce.trustAsHtml(data.content);}
$splash.open({title:data.title,message:data.content});}).error(function(){$location.url('/error/notFound');});};});;"use strict";phl.controller('LanguageController',['$translate','$scope','$cookies',function($translate,$scope,$cookies){$scope.changeLanguage=function(langKey){$translate.use(langKey);if(store.enabled){store.set('NG_TRANSLATE_LANG_KEY',langKey);}
