...
$rootScope.$on('$translatePartialLoaderStructureChanged',function(){$translate.refresh();});}]);})(angular);;"use strict";(function(angular){phl.constant('ROUTES',(function(){return{HOME:'/',CONTACTS:'/contacts',HELP:'/help',AGREEMENT:'/agreement',ABOUT:'/about',NOT_FOUND:'/error/notfound',PROFILE:'/profile',LOGOUT:'/logout'}})());phl.constant('TEMPLATE',(function(){return{ARTICLE:'assets/frontend/phl/app/templates/index.html',ERROR:'assets/frontend/phl/app/templates/error.html',PROFILE:'assets/frontend/phl/app/templates/profile.html'}})());phl.config(['$routeProvider','$locationProvider','ROUTES','TEMPLATE',function($routeProvider,$locationProvider,ROUTES,TEMPLATE){$locationProvider.hashPrefix('!');$locationProvider.html5Mode(true);$routeProvider.when(ROUTES.HOME,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.CONTACTS,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.HELP,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.AGREEMENT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"}).when(ROUTES.ABOUT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexController"})
$routeProvider.when(ROUTES.LOGOUT,{controller:"IndexController",redirectTo:ROUTES.LOGOUT}).when(ROUTES.NOT_FOUND,{templateUrl:TEMPLATE.ERROR,controller:'IndexController'}).when(ROUTES.PROFILE,{templateUrl:TEMPLATE.PROFILE,controller:"SignController",security:false}).when(ROUTES.LOGOUT,{templateUrl:TEMPLATE.ARTICLE,controller:"SignController"}).otherwise({redirectTo:ROUTES.HOME});}]);phl.config(['$translateProvider','$translatePartialLoaderProvider',function($translateProvider,$translatePartialLoader){$translateProvider.fallbackLanguage(['en','ru','de','uk']).determinePreferredLanguage().registerAvailableLanguageKeys(['en','de','ru','uk'],{'en_US':'en','en_GB':'en','de_DE':'de','ua_UK':'uk'});$translateProvider.useLoader('$translatePartialLoader',{urlTemplate:'assets/frontend/phl/app/languages/{lang}/{part}.json'});}]);})(angular);;
angular.module('ui.splash',['ui.bootstrap']).service('$splash',['$modal','$rootScope',function($modal,$rootScope){var modalInstance;return{open:function(attrs,opts){var scope=$rootScope.$new();angular.extend(scope,attrs);opts=angular.extend(opts||{},{backdrop:false,scope:scope,templateUrl:'/assets/frontend/phl/app/templates/sign.html',windowTemplateUrl:'splash/index.html'});modalInstance=$modal.open(opts);return modalInstance;},close:function(){return modalInstance.close();}};}]).run(['$templateCache',function($templateCache){$templateCache.put('splash/index.html','<section class="splash" ng-class="{\'splash-open\': animate}" ng-style="{\'z-index\': 1000, display: \'block\'}" ng-click="close($event)">'+'  <div class="splash-inner" ng-transclude></div>'+'</section>');}]);;'use strict';(function(angular){phl.factory('AuthenticationService',['$http','$cookieStore','$rootScope',function($http,$cookieStore,$rootScope){var service={};service.Login=function(login,password,type,callback){$http.post('/sign',{login:login,password:password,type:type})
   .success(function(response){callback(response);});};service.UserApply=function(data){$rootScope.user=data.user;$http.get('/profile').success(function(response){$rootScope.title=response.title;$rootScope.user=response.user;});};service.Logout=function(){$http.get('/logout').success(function(response){$rootScope.title=response.title;delete $rootScope.user;});};return service;}]);})(angular);;"use strict";phl.controller('TopMenuController',['$scope','$location','$translatePartialLoader','$splash','$http','$sce','$rootScope','$anchorScroll',function($scope,$location,$translatePartialLoader,$splash,$http,$sce,$rootScope,$anchorScroll){$translatePartialLoader.addPart('menu');$scope.isActive=function(route){if($location.hash()!=''){$anchorScroll();return route===$location.url()+'#'+$location.hash();}
...
$scope.currentLanguage=langKey;};}]);;"use strict";phl.controller('SignController',['$scope','$rootScope','$location','AuthenticationService','$translatePartialLoader','$splash',function($scope,$rootScope,$location,AuthenticationService,$translatePartialLoader,$splash){$translatePartialLoader.addPart('sign');$scope.typeSign=function(string){$scope.type=string;};$scope.signIn=function(){$scope.dataLoading=true;AuthenticationService.Login($scope.login,$scope.password,$scope.type,function(response){if(response.success){AuthenticationService.UserApply(response);$splash.close();$location.path('/profile');}else{$scope.error=response.message;$scope.dataLoading=false;}});};$scope.logout=function(){AuthenticationService.Logout();$location.path('/');};}]);;