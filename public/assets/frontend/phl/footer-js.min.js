...
$rootScope.$on('$translatePartialLoaderStructureChanged',function(){$translate.refresh();});$rootScope.$on("$locationChangeSuccess",function(event,next,current){if(!Authentication.isLoggedIn()){$http.defaults.headers.common['X-Token']=$cookies.token;}});}]);})(angular);;"use strict";(function(angular){app.constant('ROUTES',(function(){return{HOME:'/',CONTACTS:'/about/contacts',HELP:'/about/help',AGREEMENT:'/about/agreement',ABOUT:'/about',NOT_FOUND:'/error/notfound',ACCOUNT:'/account',VERIFY:'/sign/verify',LOGIN:'/sign/login',REGISTER:'/sign/register',RESTORE:'/sign/restore',LOGOUT:'/sign/logout'}})());app.constant('TEMPLATE',(function(){return{ARTICLE:'assets/frontend/phl/app/common/templates/index.html',ERROR:'assets/frontend/phl/app/common/templates/error.html',SIGN:'assets/frontend/phl/app/authenticate/templates/sign.html',ACCOUNT:'assets/frontend/phl/app/user/templates/account.html'}})());app.config(['$routeProvider','$locationProvider','ROUTES','TEMPLATE',function($routeProvider,$locationProvider,ROUTES,TEMPLATE){$locationProvider.hashPrefix('!');$locationProvider.html5Mode(true);$routeProvider.when(ROUTES.HOME,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexCtrl"}).when(ROUTES.CONTACTS,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexCtrl"}).when(ROUTES.HELP,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexCtrl"}).when(ROUTES.AGREEMENT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexCtrl"}).when(ROUTES.ABOUT,{templateUrl:TEMPLATE.ARTICLE,controller:"IndexCtrl"})
...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(data){$rootScope.title=data.title;$rootScope.content=data.content;$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;'use strict';(function(angular){app.service('Authentication',['$rootScope','$q','$http','base64','Session',function($rootScope,$q,$http,base64,Session){var isAuthenticated=false;var user=null;return{requestUser:function(route){var deferred=$q.defer();$http.get(route,{headers:{'X-Token':store.get('token')}}).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;Session.set('token',base64.encode(response.token));}
