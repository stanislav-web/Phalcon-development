...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(data){$rootScope.title=data.title;$rootScope.content=data.content;$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;'use strict';(function(angular){app.service('Authentication',['$rootScope','$q','$http','base64','$cookies',function($rootScope,$q,$http,base64){var isAuthenticated=false;var user=null;return{requestUser:function(route){var deferred=$q.defer();$http.get(route,{headers:{'X-Token':store.get('token')}}).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;store.set('token',base64.encode(response.token));}
deferred.resolve(user);}).error(function(error){deferred.reject(error);});return deferred.promise;},getUser:function(){return user;},isLoggedIn:function(){return(isAuthenticated===true)?true:false;},sign:function(credentials,route){var deferred=$q.defer();$http.post(route,credentials).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;store.set('token',base64.encode(response.token));deferred.resolve(response);}
