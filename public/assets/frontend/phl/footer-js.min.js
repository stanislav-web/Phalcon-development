"use strict";var app;var spinnerModule;var splashModule;(function(angular){spinnerModule=angular.module('ngLoadingSpinner',['angularSpinner']);splashModule=angular.module('ui.splash',['ui.bootstrap']);app=angular.module('app',['ngRoute','ngAnimate','ngSanitize','ngLoadingSpinner','pascalprecht.translate','ngCookies','ui.splash','ab-base64',function($httpProvider){$httpProvider.defaults.headers.post['Content-Type']='application/x-www-form-urlencoded;charset=utf-8';$httpProvider.defaults.headers.common['X-Requested-With']='XMLHttpRequest';$httpProvider.defaults.transformRequest=[function(data){if(data===undefined){data={};}
var token=$('meta[name=token]');app.value('token_id',token.attr('title'));app.value('token_val',token.attr('content'));data[token.attr('title')]=token.attr('content');return angular.isObject(data)&&String(data)!=='[object File]'?$.param(data):data;}];}]);app.service('Meta',function(){var metaDescription='';var metaKeywords='';return{metaDescription:function(){return metaDescription;},metaKeywords:function(){return metaKeywords;},reset:function(){metaDescription='';metaKeywords='';},setMetaDescription:function(newMetaDescription){metaDescription=newMetaDescription;},appendMetaKeywords:function(newKeywords){for(var key in newKeywords){if(metaKeywords===''){metaKeywords+=newKeywords[key].name;}else{metaKeywords+=', '+newKeywords[key].name;}}}};});app.run(['$rootScope','ROUTES','$translate','$cookies','Authentication','Session','$http',function($rootScope,ROUTES,$translate,$cookies,Authentication,Session,$http){$rootScope.ROUTES=ROUTES;if($cookies){$rootScope.currentLanguage=$cookies.NG_TRANSLATE_LANG_KEY||'ru';}
else{$rootScope.currentLanguage=Session.get('NG_TRANSLATE_LANG_KEY')||'ru';}
$rootScope.$on('$translatePartialLoaderStructureChanged',function(){$translate.refresh();});$rootScope.$on("$locationChangeSuccess",function(event,next,current){if(!Authentication.isLoggedIn()){$http.defaults.headers.common['X-Token']=Session.get('token');}});}]);})(angular);;"use strict";(function(angular){app.constant('ROUTES',(function(){return{HOME:'/',PAGES:'/page/:page',NOT_FOUND:'/error/notfound',SERVER_ERROR:'/error/uncaughtexception',ACCOUNT:'/account',VERIFY:'/sign/verify',LOGIN:'/sign/login',REGISTER:'/sign/register',RESTORE:'/sign/restore',LOGOUT:'/sign/logout'};})());app.constant('TEMPLATE',(function(){return{PAGES:'assets/frontend/phl/app/common/templates/index.html',ERROR:'assets/frontend/phl/app/common/templates/error.html',SIGN:'assets/frontend/phl/app/authenticate/templates/sign.html',ACCOUNT:'assets/frontend/phl/app/user/templates/account.html'};})());app.config(['$httpProvider',function($httpProvider){$httpProvider.interceptors.push('httpRequestInterceptor');}]);app.config(['$routeProvider','$locationProvider','ROUTES','TEMPLATE',function($routeProvider,$locationProvider,ROUTES,TEMPLATE){$locationProvider.hashPrefix('!');$locationProvider.html5Mode(true);$routeProvider.when(ROUTES.HOME,{templateUrl:TEMPLATE.PAGES,controller:"IndexCtrl"}).when(ROUTES.PAGES,{templateUrl:TEMPLATE.PAGES,controller:"IndexCtrl"}).when(ROUTES.LOGOUT,{controller:"IndexCtrl",redirectTo:ROUTES.LOGOUT}).when(ROUTES.ACCOUNT,{templateUrl:TEMPLATE.ACCOUNT,controller:"UserCtrl",caseInsensitiveMatch:true,resolve:{isAuthenticated:function(Authentication){Authentication.requestUser(ROUTES.VERIFY);}}}).when('/transactions/:name',{templateUrl:function(urlattr){return'assets/frontend/phl/app/transactions/templates/'+urlattr.name+'.html';},caseInsensitiveMatch:true,controller:"UserCtrl",resolve:{isAuthenticated:function(Authentication){Authentication.requestUser(ROUTES.VERIFY);}}}).otherwise({templateUrl:TEMPLATE.ERROR,controller:'IndexCtrl'});}]);app.config(['$translateProvider','$translatePartialLoaderProvider',function($translateProvider,$translatePartialLoader){$translateProvider.fallbackLanguage(['en','ru','de','uk']).determinePreferredLanguage().registerAvailableLanguageKeys(['en','de','ru','uk'],{'en_US':'en','en_GB':'en','de_DE':'de','ua_UK':'uk','ru-RU':'ru'});$translateProvider.useLoader('$translatePartialLoader',{urlTemplate:'assets/frontend/phl/app/languages/{lang}/{part}.json'});}]);})(angular);;"use strict";(function(){spinnerModule.directive('usSpinner',['$http','$rootScope',function($http,$rootScope){return{link:function(scope,elm,attrs)
...
{$rootScope.spinnerActive=loading;if(loading){elm.removeClass('ng-hide');}else{elm.addClass('ng-hide');}});}};}]);}).call(this);;'use strict';(function(angular){app.factory('httpRequestInterceptor',['$q','$location','ROUTES',function($q,$location,ROUTES){return{'responseError':function(rejection){if(rejection.status===404){$location.path(ROUTES.NOT_FOUND);}
else if(rejection.status===500){$location.path(ROUTES.SERVER_ERROR);}
return $q.reject(rejection);}};}]);})(angular);;'use strict';(function(angular){splashModule.service('$splash',['$modal','$rootScope','TEMPLATE',function($modal,$rootScope,TEMPLATE){var modalInstance;return{open:function(attrs,opts){var scope=$rootScope.$new();angular.extend(scope,attrs);opts=angular.extend(opts||{},{backdrop:false,scope:scope,templateUrl:TEMPLATE.SIGN,windowTemplateUrl:'splash/index.html'});modalInstance=$modal.open(opts);return modalInstance;},close:function(){return modalInstance.close();}};}]).run(['$templateCache',function($templateCache){$templateCache.put('splash/index.html','<section class="splash" ng-class="{\'splash-open\': animate}" ng-style="{\'z-index\': 1000, display: \'block\'}" ng-click="close($event)">'+'  <div class="splash-inner" ng-transclude></div>'+'</section>');}]);})(angular);;'use strict';(function(angular){app.service('Session',function(){return{set:function(key,value){return store.set(key,value);},get:function(key){return store.get(key);},remove:function(key){return store.remove(key);},destroy:function(){return store.clear();}};});})(angular);;"use strict";(function(angular){app.controller('TopMenuCtrl',['$scope','$location','$translatePartialLoader','$splash','$http','$sce','$rootScope','$anchorScroll',function($scope,$location,$translatePartialLoader,$splash,$http,$sce,$rootScope,$anchorScroll){$translatePartialLoader.addPart('menu');$scope.isActive=function(route){if($location.hash()!=''){$anchorScroll();return route===$location.url()+'#'+$location.hash();}
...
$scope.currentLanguage=langKey;};}]);})(angular);;"use strict";(function(angular){app.controller('IndexCtrl',['$scope','$rootScope','$http','$location','$sce',function($scope,$rootScope,$http,$location,$sce){$http.get($location.url()).success(function(data){$scope.trusty=function(){return $sce.trustAsHtml(data.content);}
$rootScope.title=data.title;$rootScope.content=data.content;$rootScope.topmenu=data.topmenu;});}]);})(angular);;'use strict';(function(angular){app.service('Authentication',['$rootScope','$q','$http','base64','Session',function($rootScope,$q,$http,base64,Session){var isAuthenticated=false;var user=null;return{requestUser:function(route){var deferred=$q.defer();$http.get(route,{headers:{'X-Token':store.get('token')}}).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;Session.set('token',base64.encode(response.token));}
deferred.resolve(user);}).error(function(error){deferred.reject(error);});return deferred.promise;},getUser:function(){return user;},isLoggedIn:function(){return(isAuthenticated===true)?true:false;},sign:function(credentials,route){var deferred=$q.defer();$http.post(route,credentials).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;Session.set('token',base64.encode(response.token));deferred.resolve(response);}
else{deferred.resolve(response);}}).error(function(error){deferred.reject(error);});return deferred.promise;},restore:function(credentials,route){var deferred=$q.defer();$http.post(route,credentials).success(function(response){if(response.success){deferred.resolve(response);}
else{deferred.resolve(response);}}).error(function(error){deferred.reject(error);});return deferred.promise;},logout:function(route){var deferred=$q.defer();$http.delete(route).success(function(response){if(response.success){$rootScope.user=user=null;$rootScope.isAuthenticated=isAuthenticated=false;Session.remove('token');deferred.resolve(true);}
...
else{$scope.registerForm=true;$scope.restoreForm=false;$scope.loginForm=false;}};$scope.sign=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'password':$scope.password};Authentication.sign(credentials,ROUTES.LOGIN).then(function(response){if(response.success){$splash.close();$location.path(ROUTES.ACCOUNT);}},function(error){$scope.signError=error.message;$scope.dataLoading=false;});};$scope.restore=function(){$scope.dataLoading=true;var credentials={'login':$scope.login};Authentication.restore(credentials,ROUTES.RESTORE).then(function(response){$scope.restoreSuccess=response.message;$scope.dataLoading=false;setTimeout(function(){$splash.close();},3000);},function(error){$scope.restoreError=error.message;$scope.dataLoading=false;});};$scope.register=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'name':$scope.name,'password':$scope.password};Authentication.sign(credentials,ROUTES.REGISTER).then(function(response){$splash.close();$location.path(ROUTES.ACCOUNT);},function(error){$scope.registerError=error.message;$scope.dataLoading=false;});};$scope.logout=function(){Authentication.logout(ROUTES.LOGOUT).then(function(response){if(response){$location.path(ROUTES.HOME);}});};$scope.checkPassword=function(){$scope.formRegister.passwordx.$error.dontMatch=$scope.password!==$scope.passwordx;};}]);})(angular);;"use strict";"use strict";(function(angular){app.controller('UserCtrl',['$scope','$rootScope','$http','$location','ROUTES','$cookies',function($scope,$rootScope,$http,$location,ROUTES,$cookies){$http.get($location.url(),{headers:{'X-Token':$cookies.token}}).success(function(data){$rootScope.title=data.title;$rootScope.user=data.user;}).error(function(data){$rootScope.title=data.title;$rootScope.content=data.content;$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;
