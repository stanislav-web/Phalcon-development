...
Authentication.sign(credentials,ROUTES.RESTORE).then(function(response){if(response.success){$scope.restoreSuccess=response.message;setTimeout(function(){$splash.close();},3000);}else{$scope.restoreError=response.message;$scope.dataLoading=false;}});};$scope.register=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'name':$scope.name,'password':$scope.password}
