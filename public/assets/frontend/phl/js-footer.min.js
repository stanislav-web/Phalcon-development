...
else{deferred.resolve(response);}}).error(function(error){deferred.reject(error);});return deferred.promise;},logout:function(route){var deferred=$q.defer();$http.get(route).success(function(response){if(response.success){$rootScope.user=user=null;$rootScope.isAuthenticated=isAuthenticated=false;$http.defaults.headers.common['X-Token']='';deferred.resolve(true);}
...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;"use strict";(function(angular){app.controller('SignCtrl',['$scope','$rootScope','$location','Authentication','$translatePartialLoader','$splash','ROUTES',function($scope,$rootScope,$location,Authentication,$translatePartialLoader,$splash,ROUTES){$scope.loginForm=true;$scope.registerForm=false;$scope.remindForm=false;$scope.toggle=function(form){if(form==='loginForm'){$scope.registerForm=false;$scope.remindForm=false;$scope.loginForm=true;}
else if(form==='remindForm'){$scope.registerForm=false;$scope.remindForm=true;$scope.loginForm=false;}
else{$scope.registerForm=true;$scope.remindForm=false;$scope.loginForm=false;}};$scope.sign=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'password':$scope.password}
Authentication.sign(credentials,ROUTES.LOGIN).then(function(response){if(response.success){$splash.close();$location.path(ROUTES.ACCOUNT);}else{$scope.signError=response.message;$scope.dataLoading=false;}});};$scope.remind=function(){$scope.dataLoading=true;var credentials={'login':$scope.login}
Authentication.sign(credentials,ROUTES.REMIND).then(function(response){if(response.success){console.log('OK');}else{$scope.remindError=response.message;$scope.dataLoading=false;}});};$scope.register=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'name':$scope.name,'password':$scope.password}
Authentication.sign(credentials,ROUTES.REGISTER).then(function(response){if(response.success){$splash.close();$location.path(ROUTES.ACCOUNT);}else{$scope.registerError=response.message;$scope.dataLoading=false;}});};$scope.logout=function(){Authentication.logout(ROUTES.LOGOUT).then(function(response){if(response){$location.path(ROUTES.HOME);}});};$scope.checkPassword=function(){$scope.formRegister.passwordx.$error.dontMatch=$scope.password!==$scope.passwordx;};}]);})(angular);;"use strict";"use strict";(function(angular){app.controller('UserCtrl',['$scope','$rootScope','$http','$location','ROUTES',function($scope,$rootScope,$http,$location,ROUTES){$http.get($location.url()).success(function(data){$rootScope.title=data.title;$rootScope.user=data.user;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;
