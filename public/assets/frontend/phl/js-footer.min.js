...
{$rootScope.spinnerActive=loading;if(loading){elm.removeClass('ng-hide');}else{elm.addClass('ng-hide');}});}};}]);}).call(this);;'use strict';(function(angular){splashModule.service('$splash',['$modal','$rootScope',function($modal,$rootScope){var modalInstance;return{open:function(attrs,opts){var scope=$rootScope.$new();angular.extend(scope,attrs);opts=angular.extend(opts||{},{backdrop:false,scope:scope,templateUrl:'/assets/frontend/phl/app/authenticate/templates/sign.html',windowTemplateUrl:'splash/index.html'});modalInstance=$modal.open(opts);return modalInstance;},close:function(){return modalInstance.close();}};}]).run(['$templateCache',function($templateCache){$templateCache.put('splash/index.html','<section class="splash" ng-class="{\'splash-open\': animate}" ng-style="{\'z-index\': 1000, display: \'block\'}" ng-click="close($event)">'+'  <div class="splash-inner" ng-transclude></div>'+'</section>');}]);})(angular);;'use strict';(function(angular){app.service('Authentication',['$rootScope','$q','$http','$timeout','ROUTES','$cookies',function($rootScope,$q,$http,$timeout,ROUTES,$cookies){var isAuthenticated=false;var user=null;return{requestUser:function(){var deferred=$q.defer();$http.get(ROUTES.VERIFY).success(function(response){$timeout(function(){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;$http.defaults.headers.common['X-Token']=$cookies.token;}
deferred.resolve(user);},1000);}).error(function(error){deferred.reject(error);});return deferred.promise;},getUser:function(){return user;},isLoggedIn:function(){return(isAuthenticated)?true:false;},login:function(credentials){var deferred=$q.defer();$http.post(ROUTES.LOGIN,credentials).success(function(response){if(response.success){$rootScope.user=user=response.user;$rootScope.isAuthenticated=isAuthenticated=true;$http.defaults.headers.common['X-Token']=$cookies.token;deferred.resolve(response);}
else{deferred.resolve(response);}}).error(function(error){deferred.reject(error);});return deferred.promise;},logout:function(){var deferred=$q.defer();$http.get(ROUTES.LOGOUT).success(function(response){if(response.success){$rootScope.user=user=null;$rootScope.isAuthenticated=isAuthenticated=false;$http.defaults.headers.common['X-Token']='';deferred.resolve(true);}
else{deferred.resolve(false);}}).error(function(error){deferred.reject(error);});return deferred.promise;}}}]);})(angular);;"use strict";(function(angular){app.controller('TopMenuCtrl',['$scope','$location','$translatePartialLoader','$splash','$http','$sce','$rootScope','$anchorScroll',function($scope,$location,$translatePartialLoader,$splash,$http,$sce,$rootScope,$anchorScroll){$translatePartialLoader.addPart('menu');$scope.isActive=function(route){if($location.hash()!=''){$anchorScroll();return route===$location.url()+'#'+$location.hash();}
...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(){$location.url('/error/notFound');});}]);})(angular);;"use strict";(function(angular){app.controller('SignCtrl',['$scope','$rootScope','$location','Authentication','$translatePartialLoader','$splash',function($scope,$rootScope,$location,Authentication,$translatePartialLoader,$splash){var type='';$scope.typeSign=function(string){type=string;};$scope.sign=function(){$scope.dataLoading=true;var credentials={'type':type,'login':$scope.login,'password':$scope.password}
Authentication.login(credentials).then(function(response){if(response.success){$splash.close();$location.path('/account');}else{$scope.error=response.message;$scope.dataLoading=false;}});};$scope.logout=function(){Authentication.logout().then(function(response){if(response){$location.path('/');}});};}]);})(angular);;"use strict";"use strict";(function(angular){app.controller('UserCtrl',['$scope','$rootScope','$http','$location',function($scope,$rootScope,$http,$location){$http.get($location.url()).success(function(data){$rootScope.title=data.title;$rootScope.user=data.user;}).error(function(){$location.url('/error/notFound');});}]);})(angular);;
