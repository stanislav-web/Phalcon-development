...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;"use strict";(function(angular){app.controller('SignCtrl',['$scope','$rootScope','$location','Authentication','$translatePartialLoader','$splash','ROUTES',function($scope,$rootScope,$location,Authentication,$translatePartialLoader,$splash,ROUTES){$scope.loginForm=true;$scope.registerForm=false;$scope.toggle=function(form){if(form==='loginForm'){$scope.registerForm=false;$scope.loginForm=true;}
else{$scope.registerForm=true;$scope.loginForm=false;}};$scope.sign=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'password':$scope.password}
Authentication.login(credentials).then(function(response){if(response.success){$splash.close();$location.path(ROUTES.ACCOUNT);}else{$scope.error=response.message;$scope.dataLoading=false;}});};$scope.register=function(){$scope.dataLoading=true;var credentials={'login':$scope.login,'password':$scope.password}
Authentication.login(credentials).then(function(response){if(response.success){$splash.close();$location.path(ROUTES.ACCOUNT);}else{$scope.error=response.message;$scope.dataLoading=false;}});};$scope.logout=function(){Authentication.logout().then(function(response){if(response){$location.path(ROUTES.HOME);}});};}]);})(angular);;"use strict";"use strict";(function(angular){app.controller('UserCtrl',['$scope','$rootScope','$http','$location','ROUTES',function($scope,$rootScope,$http,$location,ROUTES){$http.get($location.url()).success(function(data){$rootScope.title=data.title;$rootScope.user=data.user;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;
