...
$routeProvider.when(ROUTES.LOGOUT,{controller:"IndexCtrl",redirectTo:ROUTES.LOGOUT}).when(ROUTES.NOT_FOUND,{templateUrl:TEMPLATE.ERROR,controller:'IndexCtrl'}).when(ROUTES.ACCOUNT,{templateUrl:TEMPLATE.ACCOUNT,controller:"UserCtrl",caseInsensitiveMatch:true,resolve:{isAuthenticated:function(Authentication){Authentication.requestUser();}}}).otherwise({redirectTo:ROUTES.HOME});}]);app.config(['$translateProvider','$translatePartialLoaderProvider',function($translateProvider,$translatePartialLoader){$translateProvider.fallbackLanguage(['en','ru','de','uk']).determinePreferredLanguage().registerAvailableLanguageKeys(['en','de','ru','uk'],{'en_US':'en','en_GB':'en','de_DE':'de','ua_UK':'uk','ru-RU':'ru'});$translateProvider.useLoader('$translatePartialLoader',{urlTemplate:'assets/frontend/phl/app/languages/{lang}/{part}.json'});}]);})(angular);;"use strict";(function(){spinnerModule.directive('usSpinner',['$http','$rootScope',function($http,$rootScope){return{link:function(scope,elm,attrs)
...
$rootScope.title=data.title;$rootScope.topmenu=data.topmenu;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;"use strict";(function(angular){app.controller('SignCtrl',['$scope','$rootScope','$location','Authentication','$translatePartialLoader','$splash',function($scope,$rootScope,$location,Authentication,$translatePartialLoader,$splash){var type='';$scope.typeSign=function(string){type=string;};$scope.sign=function(){$scope.dataLoading=true;var credentials={'type':type,'login':$scope.login,'password':$scope.password}
Authentication.login(credentials).then(function(response){if(response.success){$splash.close();$location.path('/account');}else{$scope.error=response.message;$scope.dataLoading=false;}});};$scope.logout=function(){Authentication.logout().then(function(response){if(response){$location.path('/');}});};}]);})(angular);;"use strict";"use strict";(function(angular){app.controller('UserCtrl',['$scope','$rootScope','$http','$location','ROUTES',function($scope,$rootScope,$http,$location,ROUTES){$http.get($location.url()).success(function(data){$rootScope.title=data.title;$rootScope.user=data.user;}).error(function(){$location.url(ROUTES.NOT_FOUND);});}]);})(angular);;
