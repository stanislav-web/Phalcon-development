AmCharts.GaugeAxis = AmCharts.Class({
    construct: function (a) {
        ...
        var h = this.minorTickInterval;
        isNaN(h) && (h = f / 5);
        var p = this.startAngle, k = this.endAngle, e = this.tickLength, l = (d - c) / f + 1, g = (k - p) / (l - 1);
        this.singleValueAngle = d = g / f;
        var m = a.container, q = this.tickColor,
            u = this.tickAlpha, C = this.tickThickness, D = f / h, G = g / D, h = this.minorTickLength, I = this.labelFrequency, s = this.radiusReal;
        this.inside || (s -= 15);
        ...
        this.gridInside ? A = n = s : (n = s - e, A = n + h);
        var r = this.axisThickness / 2, k = AmCharts.wedge(m, y, z, p, k - p, n + r, n + r, n - r, 0,
            J);
        b.push(k);
        ...
        for (n = 0; n < l; n++) {
            var v = c + n * f, r = p + n * g, w = k(y + s * Math.sin(r / 180 * Math.PI)), B = k(z - s * Math.cos(r / 180 * Math.PI)), x = k(y + (s - e) * Math.sin(r / 180 * Math.PI)), t = k(z - (s - e) * Math.cos(r / 180 * Math.PI)), w = AmCharts.line(m, [w, x], [B, t], q, u, C, 0, !1, !1, !0);
            b.push(w);
            w = -1;
            x = this.labelOffset;
            this.inside || (x = -x - e, w = 1);
            ...
            if (0 < I && n / I == Math.round(n / I) && (this.showLastLabel || n != l - 1) && (this.showFirstLabel || 0 !== n)) {
                var F;
                F = this.usePrefixes ? AmCharts.addPrefix(v, a.prefixesOfBigNumbers, a.prefixesOfSmallNumbers, a.nf, !0) : AmCharts.formatNumber(v, a.nf, J);
                var H = this.unit;
                H && (F = "left" == this.unitPosition ? H + F : F + H);
                (H = this.labelFunction) && (F = H(v));
                v = AmCharts.text(m, F, a.color, a.fontFamily, E);
                E = v.getBBox();
                v.translate(B + w * E.width / 2 * K, x + w * E.height / 2 * t);
                b.push(v)
            }
            if (n < l -
                1)for (v = 1; v < D; v++)t = r + G * v, w = k(y + A * Math.sin(t / 180 * Math.PI)), B = k(z - A * Math.cos(t / 180 * Math.PI)), x = k(y + (A - h) * Math.sin(t / 180 * Math.PI)), t = k(z - (A - h) * Math.cos(t / 180 * Math.PI)), w = AmCharts.line(m, [w, x], [B, t], q, u, C, 0, !1, !1, !0), b.push(w)
        }
        if (b = this.bands)for (c = 0; c < b.length; c++)if (f = b[c])q = f.startValue, u = f.endValue, e = AmCharts.toCoordinate(f.radius, s), isNaN(e) && (e = A), l = AmCharts.toCoordinate(f.innerRadius, s), isNaN(l) && (l = e - h), g = p + d * (q - this.startValue), u = d * (u - q), C = f.outlineColor, void 0 == C && (C = this.bandOutlineColor),
            D = f.outlineThickness, isNaN(D) && (D = this.bandOutlineThickness), G = f.outlineAlpha, isNaN(G) && (G = this.bandOutlineAlpha), q = f.alpha, isNaN(q) && (q = this.bandAlpha), e = AmCharts.wedge(m, y, z, g, u, e, e, l, 0, {
            fill: f.color,
            stroke: C,
            "stroke-width": D,
            "stroke-opacity": G
...
AmCharts.AmAngularGauge = AmCharts.Class({
    inherits: AmCharts.AmChart, construct: function (a) {
        ...
        this.realHeight = c;
        var d = AmCharts.toCoordinate, f = d(this.marginLeft, b), h = d(this.marginRight, b), p = d(this.marginTop, c) + this.getTitleHeight(), k = d(this.marginBottom, c), e = d(this.radius, b, c), d = b - f - h, l = c - p - k + this.extraHeight;
        e || (e = Math.min(d, l) / 2);
        e < this.minRadius && (e = this.minRadius);
        this.radiusReal = e;
        this.centerX = (b - f - h) / 2 + f;
        ...
        for (b = e = a = 0; b < this.axes.length; b++)c = this.axes[b], f = c.radius, c.radiusReal = AmCharts.toCoordinate(f, this.radiusReal), c.draw(), h = 1, -1 !== String(f).indexOf("%") && (h = 1 + (100 - Number(f.substr(0, f.length - 1))) / 100), c.width * h > a && (a = c.width * h), c.height * h > e &&
        (e = c.height * h);
        ...
        a.set = c.set();
        if (!a.hidden) {
            var d = a.axis, f = d.radiusReal, h = d.centerXReal, p = d.centerYReal, k = a.startWidth, e = a.endWidth, l = AmCharts.toCoordinate(a.innerRadius, d.radiusReal), g = AmCharts.toCoordinate(a.radius, d.radiusReal);
            d.inside || (g -= 15);
            var m = a.nailColor;
            m || (m = a.color);
            var q = a.nailColor;
            q || (q = a.color);
            m = AmCharts.circle(c, a.nailRadius,
                m, a.nailAlpha, a.nailBorderThickness, m, a.nailBorderAlpha);
            a.set.push(m);
            m.translate(h, p);
            isNaN(g) && (g = f - d.tickLength);
            var d = Math.sin(b / 180 * Math.PI), f = Math.cos(b / 180 * Math.PI), m = Math.sin((b + 90) / 180 * Math.PI), u = Math.cos((b + 90) / 180 * Math.PI), c = AmCharts.polygon(c, [h - k / 2 * m + l * d, h + g * d - e / 2 * m, h + g * d + e / 2 * m, h + k / 2 * m + l * d], [p + k / 2 * u - l * f, p - g * f + e / 2 * u, p - g * f - e / 2 * u, p - k / 2 * u - l * f], a.color, a.alpha, 1, q, a.borderAlpha, void 0, !0);
            a.set.push(c);
