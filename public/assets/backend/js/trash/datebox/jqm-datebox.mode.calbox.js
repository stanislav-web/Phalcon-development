...
(function ($) {
    ...
    $.extend($.mobile.datebox.prototype._build, {
        'calbox': function () {
            ...
            cal = {
                'today': -1, 'highlightDay': -1, 'presetDay': -1, 'startDay': w.__('calStartDay'),
            ...
            if (cal.thisDateArr[0] === cal.theDateArr[0] && cal.thisDateArr[1] === cal.theDateArr[1]) {
            ...
            if (o.minDays !== false) {
                ...
                if (cal.theDateArr[0] === cal.minDate.getFullYear() && cal.theDateArr[1] <= cal.minDate.getMonth()) {
                    w.calPrev = false;
                }
            }
            if (o.maxDays !== false) {
                cal.maxDate.adj(2, o.maxDays);
                if (cal.theDateArr[0] === cal.maxDate.getFullYear() && cal.theDateArr[1] >= cal.maxDate.getMonth()) {
                    w.calNext = false;
                }
            }
            ...
            for (var row = 0, rows = cal.gen.length; row < rows; row++) {
                ...
                if (o.calShowWeek) {
                    ...
                    cal.wk++;
                    if (cal.wk > 52 && typeof cal.gen[parseInt(row, 10) + 1] !== 'undefined') {
                        cal.wk = new Date(cal.theDateArr[0], cal.theDateArr[1], ((w.__('calStartDay') === 0) ? cal.gen[parseInt(row, 10) + 1][1][0] : cal.gen[parseInt(row, 10) + 1][0][0])).getDWeek(4);
                    }
                }
                for (var col = 0, cols = cal.gen[row].length; col < cols; col++) {
                    if (o.calWeekMode) {
                        cal.weekMode = cal.gen[row][o.calWeekModeDay][0];
                    }
            ...
            if (o.calShowWeek) {
                temp.find('.' + uid + 'griddate').addClass(uid + 'griddate-week');
            }
