...
function LoadLeafletScript(callback){
	if (!$.fn.L){
		$.getScript('/assets/plugins/leaflet/leaflet.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('/assets/plugins/fineuploader/jquery.fineuploader-5.0.5.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	...
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('/assets/plugins/d3/d3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadAmchartsScripts(callback){
	function LoadAmchartsScript(){
		$.getScript('/assets/plugins/amcharts/amcharts.js', LoadFunnelScript);
	}
	function LoadFunnelScript(){
		$.getScript('/assets/plugins/amcharts/funnel.js', LoadSerialScript);
	}
	function LoadSerialScript(){
		$.getScript('/assets/plugins/amcharts/serial.js', LoadPieScript);
	}
...
function LoadChartistScripts(callback){
	function LoadChartistScript(){
		$.getScript('/assets/plugins/chartist/chartist.min.js', callback);
	}
	if (!$.fn.Chartist){
		LoadChartistScript();
	}
...
function LoadSpringyScripts(callback){
	function LoadSpringyScript(){
		$.getScript('/assets/plugins/springy/springy.js', LoadSpringyUIScript);
	}
	function LoadSpringyUIScript(){
		$.getScript('/assets/plugins/springy/springyui.js', callback);
	}
	if (!$.fn.Springy){
		LoadSpringyScript();
	}
